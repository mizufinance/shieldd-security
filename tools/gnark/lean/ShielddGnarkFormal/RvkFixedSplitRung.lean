...
/-! Split fixed-base RVK rung bridge.

Rows after the first 149 fused RVK rungs rematerialize each fixed-base add as a
six-row native-add block followed by two select rows. This lemma promotes one
such block to the same `FixedStepRel` used by the fused-rung keystone.
-/

namespace Shieldd.GnarkFormal.RvkFixedSplitRung
...
/-- Base-agnostic split-rung algebra shared by the RVK and net-balance fixed
walks. `Ci` and the seed coordinates are bound by the caller. -/
theorem splitRung_stepRel
  ...
  have e69x : akY * (gx*b0 + d1x) + akX * (1 + gym1*b0 + d1y) = i69 := by
    rw [e69]
    ring
  refine RvkFixedBaseRung.fixedRung_stepRel bit
  ...
  · rw [hC]
    simp only
    rw [e68x]
    have h18' : outY * (1 - i68) = i71 + Ev*i67 := by linear_combination h18
...
theorem splitRung_fixedStepRel
    ...
    (b0 d1x d1y bit i67 i68 i69 i71 outX outY sdx sdy : F)
    (hp : onCurve ⟨GX*b0 + d1x, 1 + GYM1*b0 + d1y⟩)
    (hC : RvkFixedBaseConstants.C i = ⟨akX, akY⟩)
    (cLa : la = d * akY)
    (cLb : lb = la * GX)
    (cRb : rb = akX * GYM1)
    (cC : cc = akX * GYM1 + akY * GX)
    ...
    (hSelY : (1*bit) * ((-1) + negGY*b0 + (-1)*d1y + outY) = sdy)
    (hbit : (1*bit) * (1 + (-1)*bit) = 0) :
    RvkFixedBaseLadder.FixedStepRel i bit
  ...
  have e69x : akY * (GX*b0 + d1x) + akX * (1 + GYM1*b0 + d1y) = i69 := by
    rw [e69]
    ring
  refine RvkFixedBaseRung.fixedRung_fixedStepRel i bit
...
end Shieldd.GnarkFormal.RvkFixedSplitRung
