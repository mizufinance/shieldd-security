...
/-! Composable semantics for deployed fixed-base RVK state traces. -/

namespace Shieldd.GnarkFormal.RvkFixedRun

open Bool (toZMod)
open Shieldd.GnarkFormal.ScalarMulBridge (scalarMulFromBits)
open Shieldd.GnarkFormal.RvkFixedBaseConstants (C)
open Shieldd.GnarkFormal.RvkFixedBaseLadder (FixedStepRel fixedLadder)
...
theorem seedStepRel (bit : Bool) :
  ...
  · cases bit <;> simp [seedAcc, GatesDef.select,
...
theorem trace_to_fixedLadder {n : ℕ} (bits : List.Vector Bool n) (acc : ℕ → Point)
  ...
  | succ fuel ih =>
      ...
      have hnext :=
        (RvkFixedBaseLadder.fixedStep_semantic bitIndex bits[bitIndex]!
...
theorem trace_final_semantic {n : ℕ} (bits : List.Vector Bool n) (acc : ℕ → Point)
  ...
  obtain ⟨hmodelOn, hx, hy⟩ :=
    RvkFixedBaseLadder.fixedLadder_final_coords bits (acc n).x (acc n).y
...
end Shieldd.GnarkFormal.RvkFixedRun
