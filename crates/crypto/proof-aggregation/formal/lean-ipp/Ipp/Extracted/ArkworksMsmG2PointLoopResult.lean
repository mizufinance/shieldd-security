import Ipp.Extracted.ArkworksMsmG2PointLoop

/-! Finite-execution refinement of G2 point-to-bucket accumulation. -/
...
/-- The generated G2 point loop realizes `processBuckets` on the remaining pairs. -/
theorem valid_g2_process_buckets
    ...
    (output : alloc.vec.Vec
      ark_ip_proofs.s3_07_arkworks_fq_spike.G2ProjMont)
    (hexec :
      ark_ip_proofs.s3_07_arkworks_fq_spike.g2_msm_unchecked_loop1_loop0
    ...
    output.val.length = bucketCount ∧
      ∀ index (hindex : index < bucketCount)
        ...
        ValidG2LoopState output.val[index]
  ...
  have go : ∀ {state final},
      ...
      ∀ (currentModel : Buckets (G := G2AffinePoint) bucketCount),
        state.1.val.length = bucketCount →
        (∀ index (hindex : index < bucketCount)
            (hraw : index < state.1.val.length),
          ValidG2LoopState state.1.val[index]
        ...
        ∀ out, final = .ok out →
          out.val.length = bucketCount ∧
            ∀ index (hindex : index < bucketCount)
              ...
              ValidG2LoopState out.val[index]
    ...
    | next hbody hnext ih =>
        ...
        let pair : WnafDigit × G2AffinePoint :=
        ...
        have hstep := valid_g2_point_body_step
...
#print axioms valid_g2_process_buckets
