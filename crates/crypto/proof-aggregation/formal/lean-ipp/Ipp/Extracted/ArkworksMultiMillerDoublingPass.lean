...
/-! Fuel induction for one complete doubling-line pass over a chunk. -/

namespace Ipp.Extracted.ArkworksMultiMillerDoublingPass
...
private abbrev doublingBody (filtered : List ExecPair) (chunkStart : Nat) :=
...
private theorem doublingTraceCore
    ...
    {state result} (hrun : LoopResult (doublingBody filtered chunkStart)
      state result) :
  ...
  | @next state next result hstep hnext ih =>
      ...
      have hactive : processed < width := by
        ...
        rw [if_neg (show ¬(⟨processed⟩ : Usize) < ⟨width⟩ by
          exact hnot)] at hstep
        cases hstep
      have hnextInv := doubling_line_step filtered chunkStart width cursor
...
/-- A successful doubling-line loop advances every chunk cursor once. -/
theorem doubling_line_pass
    (filtered : List ExecPair) (chunkStart width cursor : Nat)
    (baseValues : List Ipp.Bls12377.Fq12Model)
  ...
  exact doublingTraceCore filtered chunkStart width cursor baseValues
    outCursors outF hvalid hchunk hcursor hbaseLength hrun 0 cursors f
    rfl rfl hinv

end Ipp.Extracted.ArkworksMultiMillerDoublingPass
