...
import Ipp.Extracted.ArkworksFinalExpCyclotomicSquareAlgebra0
...
local instance square0Prime : Fact q.Prime := ⟨arithmeticFacts.basePrime⟩
local instance square0Fq2Nonresidue :
    Fact (∀ x : Fq, x ^ 2 ≠ (-5) + 0 * x) :=
  ⟨by intro x; simpa using arithmeticFacts.fq2Nonresidue x⟩
local instance square0FintypeFq2 : Fintype Fq2 :=
  Fintype.ofEquiv (Fq × Fq) (QuadraticAlgebra.equivProd (-5 : Fq) 0).symm

theorem fq12CyclotomicSquare_c0c0 (a : Fq12Model)
    (hx : fq12Coefficients a ^ (q ^ 4 - q ^ 2 + 1) = 1) :
    (fq12CyclotomicSquare a).c0.c0 = (fq12Square a).c0.c0 := by
  ...
  change c = _ at hc
  have h0 := congrArg (fun x : Fq12Model => x.c0.c0) hrel
  ...
  simp only [fq12Mul, fq6Mul, fq6Add, fq6MulByV] at h0
  rw [hc, hb] at h0
  simp only [a0] at h0
  simp only [ζ_square, ζ_cube, ζ_fourth, hζ5] at h0
  exact fq12CyclotomicSquare_c0c0_algebra
    ζ r0 r4 r3 r2 r1 r5 ζ_square h0

#print axioms fq12CyclotomicSquare_c0c0
