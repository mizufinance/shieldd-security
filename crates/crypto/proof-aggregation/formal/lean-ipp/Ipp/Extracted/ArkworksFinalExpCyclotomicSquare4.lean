...
import Ipp.Extracted.ArkworksFinalExpCyclotomicSquareAlgebra4
...
local instance square4Prime : Fact q.Prime := ⟨arithmeticFacts.basePrime⟩
local instance square4Fq2Nonresidue :
    Fact (∀ x : Fq, x ^ 2 ≠ (-5) + 0 * x) :=
  ⟨by intro x; simpa using arithmeticFacts.fq2Nonresidue x⟩
local instance square4FintypeFq2 : Fintype Fq2 :=
  Fintype.ofEquiv (Fq × Fq) (QuadraticAlgebra.equivProd (-5 : Fq) 0).symm

theorem fq12CyclotomicSquare_c1c1 (a : Fq12Model)
    (hx : fq12Coefficients a ^ (q ^ 4 - q ^ 2 + 1) = 1) :
    (fq12CyclotomicSquare a).c1.c1 = (fq12Square a).c1.c1 := by
  ...
  change c = _ at hc
  have h3 := congrArg (fun x : Fq12Model => x.c1.c1) hrel
  ...
  simp only [fq12Mul, fq6Mul, fq6Add, fq6MulByV] at h3
  rw [hc, hb] at h3
  simp only [a0] at h3
  simp only [ζ_square, ζ_cube, ζ_fourth, hζ5] at h3
  exact fq12CyclotomicSquare_c1c1_algebra
    ζ r0 r4 r3 r2 r1 r5 ζ_square h3

#print axioms fq12CyclotomicSquare_c1c1
