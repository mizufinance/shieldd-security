...
/-! Finite execution of the outer G1 window-sum collection loop. -/
...
/-- Every generated G1 window is appended in low-to-high order with its model. -/
theorem valid_g1_windows
    (bases : Slice ark_ip_proofs.s3_07_arkworks_fq_spike.G1AffineMont)
    (size digitsCount : Usize)
    (allDigits : alloc.vec.Vec WnafDigit)
    (bucketCount : Usize)
    (pairs : Fin digitsCount.val → List (WnafDigit × G1AffinePoint))
  ...
  let initial :=
    alloc.vec.Vec.with_capacity
      ark_ip_proofs.s3_07_arkworks_fq_spike.G1ProjMont digitsCount
  unfold ark_ip_proofs.s3_07_arkworks_fq_spike.g1_msm_unchecked_loop1 at hexec
  have hrun := Aeneas.loopResult_of_eq (by simp) hexec
  let loopBody := fun state :
      alloc.vec.Vec ark_ip_proofs.s3_07_arkworks_fq_spike.G1ProjMont × Usize =>
  ...
  have go : ∀ {state final},
        ...
        ∀ out, final = .ok out →
          ∃ hlength : out.val.length = digitsCount.val,
            ∀ window (hwindow : window < out.val.length),
              ValidG1LoopState out.val[window]
    ...
    | next hbody hnext ih =>
        ...
        have hwindowResult := valid_g1_window_body bases size digitsCount allDigits
